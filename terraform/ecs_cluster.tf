...
resource "aws_ecr_repository" "backend_repository" {
  name         = "ecr-${var.application_key}-${var.application_stage}"
  force_delete = true
...
resource "aws_ecs_cluster" "backend_cluster" {
  name = "ecs-${var.application_key}-${var.application_stage}"
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration : {
        logDriver : "awslogs",
        options : {
          awslogs-group : "/ecs/${each.value.application_name}-${var.application_key}-${var.application_stage}",
